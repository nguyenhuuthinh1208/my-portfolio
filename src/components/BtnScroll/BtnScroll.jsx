...
function BtnScroll() {
  const [initialLocation, setInitialLocation] = useState(0);

  useEffect(() => {
    const handleScroll = () => {
      const currentLocation = window.pageYOffset;

      if (initialLocation < 200) {
        document.getElementById("move").style.opacity = "0";
        document.getElementById("move").style.overflow = "hidden";
      } else {
        document.getElementById("move").style.opacity = "100";
        document.getElementById("move").style.overflow = "visible";
      }
  ...
  return (
    <div
      id="move" className="move" data-aos="fade-up" data-aos-anchor-placement="top-bottom">
      <a href="#about" aria-label="button up">
      <svg className="rocket" width="24" height="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-label='Button Scroll' role='img'>
